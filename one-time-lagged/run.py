...
import dynotears
import utils as ut
from data_utils import generate_dataset, load_metr_la_data, get_normalized_adj, generate_dataset_new
...
def data_pre_from_file(A, X, n, d, s0, w_graph_type, p_graph_type, sem_type):
    #A:  原始数据邻接矩阵[207, 207] X: 交通流原始数据[207, 2, timelen]
    #n:  node num
    #d:  feature num per node
    #s0: w_graph p_graph edge num
    ...
    training_input_x = generate_dataset_new(train_original_data)
    ...
    for i in range(num_step):
        #Xbase1 = ut.simulate_linear_sem_with_P(w_mat, p1_mat, adj1 @ Xbase1, n, sem_type, noise_scale=1)
        Xbase.append(training_input_x[i])
                ...
                for p_graph_type in p_graph_types:
                    for sem_type in sem_types:
                        ...
                        for times in range(5):
                            ...
                            #Xlags, adj1, w_true, w_mat, p1_true, p1_mat = data_pre_from_file(A, X, n, d, s0, w_graph_type, p_graph_type, sem_type)
                            ...
                            np.savez('w1_p1.npz', w1=W_est_1, p1=P1_est_1)
                            np.savez('w2_p2.npz', w2=W_est_2, p2=P1_est_2)
                            np.savez('w3_p3.npz', w3=W_est_3, p3=P1_est_3)
                            ...
                            for i in range(3):
                                with open(re_file, 'a') as result_file:
                                    if i == 0:
                                        print("-----GraphNotears-----")
                                        result_file.write("-----GraphNotears-----"+"\n")
                                    if i == 1:
                                        print("-----notears + lasso-----")
                                ...
                                for thre in threshold:
                                    ...
                                    acc = ' fdr = ' + str(fdr) + ' tpr = ' + str(tpr) + ' fpr = ' + str(fpr) + ' shd = ' + str(shd) + ' nnz = ' + str(pred_size)
                                    string = "P1_est: threshold = " + str(thre) + " " + str(acc) + "  f1 = " + str(p1_f1_) +"\n"
                                    print(string)
                                    with open(re_file, 'a') as result_file:
                                        result_file.write(string)
